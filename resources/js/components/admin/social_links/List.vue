                ...
                <div class="container-fluid custom-table">
                    <div class="header d-none d-md-block">
                        <div class="row">
                            ...
                            <div class="no-pad col-sm-2">
                                <span class="link" v-on:click="sortBy('is_open_newtab')">
                                    Is Open New Tab
                                    <SortArrow :desc="this.desc"/>
                                </span>
                            </div>
                    ...
                    <div class="body" v-if="!this.is_loading">
                        <div v-bind:key="data.id" v-for="data in this.datas" class="row">
                            <div class="no-pad col-sm-2 m-center">
                                <ImagePreviewer :photo="data.icon_name" path="/images/social_links/" size="small"/>
                            </div>
                            <div class="no-pad col-sm-2 m-center">
                                {{ data.name }}
                            </div>
                            ...
                            <div class="no-pad col-sm-2 m-center">
                                <IsActiveDisplay :is_active="data.is_open_newtab"/>
                            </div>
                            <div class="no-pad col-sm-2 m-center">
...
export default {
    ...
    methods: {
        ...
        showDeleteConfirm($id, $name){
            ...
            if (isDelete == true) {
                //call delete API
                axios.delete('/api/admin/social_links/'+$id, userHelper.authenticationBearer())
                .then(res => {
                    if(res.status===200){
                        //check if success
        ...
        getDatas(is_load_more){
            ...
            //get users
            axios.get('/api/admin/social_links',{
                params:{
                    take: this.take,
