            ...
            <div>
                ...
                <div class="container-fluid custom-table">
                    <div class="header d-none d-md-block">
                        <div class="row">
                            <div class="no-pad col-sm-9">
                                Message
                            </div>
                            <div class="no-pad col-sm-3">
                                <span class="link" v-on:click="sortBy('created_at')">
                                    Created At
                    ...
                    <div class="body" v-if="!this.is_loading">
                        <div v-bind:key="data.id" v-for="data in this.datas" class="row">      
                            <router-link class="no-pad col-sm-9 m-center" v-if="data.is_vue_link" :to="data.link">
                                {{ data.message.length > 200 ? data.message.substr(0,200) + "..." : data.message }}
                            </router-link>
                            <a :href="data.link" class="no-pad col-sm-9 m-center link" v-if="!data.is_vue_link" >
                                {{ data.message.length > 200 ? data.message.substr(0,200) + "..." : data.message }}
                            </a>             
                            <div class="no-pad col-sm-3 m-center">
                                <span>{{ data.created_at | moment("from") }}</span>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        showDeleteConfirm($id, $name){
            ...
            if (isDelete == true) {
                //call delete API
                axios.delete('/api/admin/notifications/'+$id, userHelper.authenticationBearer())
                .then(res => {
                    if(res.status===200){
                        //check if success
